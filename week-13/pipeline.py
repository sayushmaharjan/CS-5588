...
class FashionPipeline:
    ...
    def generate_inpaint(self, reference_image, occasion, style, color_palette, seed=42, strength=0.95, **kwargs):
        ...
        # Free inpaint pipe after done to save memory
        del inpaint_pipe
        gc.collect()
        torch.mps.empty_cache()

        # ── Pixel-Space Compositing ──────────────────────────────────────────
        # To perfectly preserve the original face and background from VAE 
        # decoding loss, we composite the generated image with the pristine
        # original image using a feathered version of the segmentation mask.
        blurred_mask = mask.filter(ImageFilter.GaussianBlur(radius=3))
        
        final_images = []
        for gen_img in result.images:
            # gen_img has the new clothing. ref is the pristine original.
            # mask is white (255) for clothing, black (0) for face/bg.
            composite_img = Image.composite(gen_img, ref, blurred_mask)
            final_images.append(composite_img)

        return {"images": final_images, "mask": mask, "prompt": prompt, "reference_image": ref}
